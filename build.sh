...
# 是否开启「ech-tunnel 终极全平台模式」（50+ 文件）—— 改成 false 就回到普通模式
ECH_TUNNEL_FULL_MODE=true

# ========================================
# 1. 安装 musl 工具链（已彻底解决所有坑）
# ========================================
install_musl_toolchains() {
  ...
  for t in "${tools[@]}"; do
    ...
    local dir="$tmpdir/$(tar -tf "$tmpdir/toolchain.tgz" | head -1 | cut -d/ -f1)"
    sudo cp -r "$dir" /usr/local/
    sudo ln -sf "/usr/local/$t/bin/"* /usr/local/bin/ 2>/dev/null || true
  done
  echo "musl 工具链安装完成（tar 警告属于 GitHub Actions 正常现象，可忽略）"
}
...
# ========================================
# 3. Windows 7 兼容版（386 + amd64）—— 专为 ech-tunnel 增加
# ========================================
BuildWin7() {
  echo "=== 编译 Windows 7 兼容版（386 + amd64）==="
  local gov=$(go version | grep -o 'go1\.[0-9.]*')
  curl -fsSL --retry 3 -o go-win7.zip \
    "https://github.com/XTLS/go-win7/releases/download/patched-${gov}/go-for-win7-linux-amd64.zip"
  unzip -q go-win7.zip -d go-win7 && rm go-win7.zip
  chmod +x wrapper/zcc-win7* wrapper/zcxx-win7* 2>/dev/null || true

  for arch in 386 amd64; do
    [ "$arch" = "386" ] && c=zcc-win7-386 || c=zcc-win7
    [ "$arch" = "386" ] && cx=zcxx-win7-386 || cx=zcxx-win7
    CC="$(pwd)/$c" CXX="$(pwd)/$cx" \
      $(pwd)/go-win7/bin/go build -o "build/${appName}-windows7-${arch}.exe" -ldflags="$ldflags" .
  done
}

# ========================================
# 4. 全部 musl 静态平台（含所有冷门）
# ========================================
BuildMuslAll() {
...
case "$1" in
  release)
    case "$2" in
      ...
      *)
        BuildMain
        if $ECH_TUNNEL_FULL_MODE; then
          BuildWin7          # Windows 7 兼容版
        fi
        BuildMuslAll
        BuildAndroid
        MakeCompress
  ...
  *)
    ...
    echo "不传参数时编译全部平台（ECH_TUNNEL_FULL_MODE=$ECH_TUNNEL_FULL_MODE）"
    exit 1
    ;;
esac
