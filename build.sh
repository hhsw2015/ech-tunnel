...
BuildWin7() {
  ...
  curl -fsSL --retry 3 -o go-win7.zip $githubAuthArgs \
    "https://github.com/XTLS/go-win7/releases/download/patched-${go_version}/go-for-win7-linux-amd64.zip"
  rm -rf go-win7 && unzip go-win7.zip -d go-win7 && rm go-win7.zip
  ...
  for arch in "386" "amd64"; do
    ...
    if [ "$arch" = "386" ]; then
      ...
      export CC=$(pwd)/wrapper/zcc-win7 CXX=$(pwd)/wrapper/zcxx-win7
    fi
...
BuildLoongGLIBC() {
  ...
  if [ "$target_abi" = "abi1.0" ]; then
    curl -fsSL --retry 3 $githubAuthArgs \
...
BuildReleaseFreeBSD() {
  mkdir -p "build"
  freebsd_version=$(curl -fsSL --max-time 2 $githubAuthArgs "https://api.github.com/repos/freebsd/freebsd-src/tags" | \
...
# ====================== 主入口：完整调用所有 OpenList 函数 ======================
BuildReleaseAll() {
  rm -rf build .git && mkdir -p build

  # 1. xgo 主力平台 + FreeBSD + 所有冷门 glibc
