...
// 使用 DoH (DNS over HTTPS) 查询 HTTPS 记录
// 这样可以绕过软路由的 DNS 重定向(53端口拦截)
func queryHTTPSRecordViaDoH(domain string) (string, error) {
	// 使用 Cloudflare DoH 服务
...
func handleTCPConnection(
	...
	if tcpConnReal, ok := tcpConn.(*net.TCPConn); ok {
		...
		// 设置缓冲区大小
		_ = tcpConnReal.SetReadBuffer(262144)  // 256KB
		_ = tcpConnReal.SetWriteBuffer(262144) // 256KB
	}
	...
	go func() {
		...
		for {
			...
			// 从内存池获取缓冲区 (如果大小不匹配, 则直接创建新切片, 避免内存池污染)
			var buf []byte
			var bufPtr *[]byte
			
			if currentBufSize == 262144 {
...
func startMultiChannelTCPForwarder(listenAddress, targetAddress string, pool *ECHPool) {
	...
	for {
		...
		if tcpConnReal, ok := tcpConn.(*net.TCPConn); ok {
			...
			_ = tcpConnReal.SetKeepAlive(true)
			_ = tcpConnReal.SetKeepAlivePeriod(30 * time.Second)
			_ = tcpConnReal.SetReadBuffer(262144)
			_ = tcpConnReal.SetWriteBuffer(262144)
		}
